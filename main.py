...
class GameMacro:
    ...
    def build_status_line(self):
        display_name = 'RYL2 Return Of Comeback [ROC]'
        ...
        rows = [
            ...
            ((self.worker_manager.loop_skill_attack_on,    "Auto Skill Attack"),
            (self.worker_manager.loop_combined_action_on, "Auto Jump Attack")),
            ((self.worker_manager.loop_auto_move_on,       "Auto Move"),
            (self.worker_manager.loop_resser_on,          "Auto Resser")),
            ((self.worker_manager.loop_auto_unpack_on,     "Auto Unpack"),
            (None,                                         "")),  # pad last column
        ]

        # Fixed widths to keep the UI perfectly stable
        INDICATOR_W = 9   # length for "● [ON]" / "● [OFF]"
        LABEL_W     = 17  # length for the feature name
        CELL_W      = INDICATOR_W + 1 + LABEL_W  # indicator + space + label
        ...
        def cell(flag_and_label):
            ...
            return f"{dot} {Colors.BRIGHT_WHITE}{label:<{LABEL_W}}{Colors.RESET}"
    ...
    def change_config(self):
        while True:  # Use a loop instead of recursion to prevent stack overflow
            ...
            options = [
                ...
                ('8', 'Auto Unpack Gold', self.config['RiskYourLife-Macros']['AUTO_UNPACK']),
                ('9', 'Quit Script', self.config['RiskYourLife-Macros']['QUIT_SCRIPT']),
                ('0', 'Cancel', '')
            ]
            ...
            try:
                choice = input(f"{Colors.BRIGHT_YELLOW}Enter choice (0-9): {Colors.RESET}").strip()
                if choice == '0':
                    print()
    ...
    def run(self):
        ...
        hotkeys = [
            ...
            (self.config['RiskYourLife-Macros']['AUTO_UNPACK'], "Auto Unpack"),
